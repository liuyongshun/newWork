...
Vcity.CitySelector.prototype = {
    ...
    createWarp:function(){
        ...
        div.style.left = inputPos.left - 250 + 'px';
    ...
    linkEvent:function(){
        ...
        for(var i=0,n=links.length;i<n;i++){
            links[i].onclick = function(){
               document.getElementById('liu').innerHTML = this.innerHTML;
    ...
    inputEvent:function(){
        ...
        //     that.input.select();
        //     if(that.input.innerText == '城市名')document.getElementById('liu') = '';
        // });
        Vcity._m.on(this.input,'blur',function(){
            // if(that.input.innerText == '')document.getElementById('liu') = '城市名';
            
            var innerText = Vcity._m.trim(that.input.innerText);
            if(innerText != ''){
                var reg = new RegExp("^" + innerText + "|\\|" + innerText, 'gi');
                var flag=0;
                ...
                if(flag==0){
                   document.getElementById('liu')= '';
                    ...
                    if(typeof lis == 'object' && lis['length'] > 0){
                        ...
                        var bs = li.children;
                        if(bs && bs['length'] > 1){
                            document.getElementById('liu').innerText = bs[0].innerHTML;
                        }
                    }else{
                       document.getElementById('liu') = '';
                    }
    ...
    createUl:function () {
        ...
        var str;
        var innerText = Vcity._m.trim(this.input.innerText);
        // 当innerText不等于空的时候执行
        if (innerText !== '') {
            var reg = new RegExp("^" + innerText + "|\\|" + innerText, 'gi');
            ...
            if (searchResult.length == 0) {
                this.isEmpty = true;
                str = '<li class="empty">对不起，没有找到 "<em>' + innerText + '</em>"</li>';
                searchResult.push(str);
            }
            // 如果slideul不存在则添加ul
            if (!this.ul) {
    ...
    KeyboardEvent:function(event,keycode){
        ...
        switch(keycode){
            ...
            case 38: //向上箭头↑
                ...
                break;
            case 13: // enter键
                this.input.innerText = Vcity.regExChiese.exec(lis[this.count].innerHTML)[0];
                Vcity._m.addClass('hide',this.ul);
                Vcity._m.addClass('hide',this.ul);
                /* IE6 */
    ...
    liEvent:function(){
        ...
        for(var i = 0,n = lis.length;i < n;i++){
            Vcity._m.on(lis[i],'click',function(event){ 
                event = Vcity._m.getEvent(event);
                var target = Vcity._m.getTarget(event);
               document.getElementById('liu') = Vcity.regExChiese.exec(target.innerHTML)[0];
