...
Vcity.CitySelector.prototype = {
    ...
    linkEvent:function(){
        ...
        for(var i=0,n=links.length;i<n;i++){
            links[i].onclick = function(){
                document.getElementById('switchCity').innerText = this.innerHTML;
    ...
    inputEvent:function(){
        ...
        //     that.input.select();
        //     if(document.getElementById('switchCity').innerText == '城市名') document.getElementById('switchCity').innerText = '';
        // });
        Vcity._m.on(this.input,'blur',function(){
            // if(document.getElementById('switchCity').innerText == '') document.getElementById('switchCity').innerText = '城市名';
            
            var value = Vcity._m.trim(document.getElementById('switchCity').innerText);
            if(value != ''){
                ...
                if(flag==0){
                     document.getElementById('switchCity').innerText = '';
                }else{
                    ...
                    if(typeof lis == 'object' && lis['length'] > 0){
                        ...
                        var bs = li.children;
                        if(bs && bs['length'] > 1){
                            document.getElementById('switchCity').innerText = bs[0].innerHTML;
                        }
                    }else{
                         document.getElementById('switchCity').innerText = '';
                    }
    ...
    liEvent:function(){
        ...
        for(var i = 0,n = lis.length;i < n;i++){
            Vcity._m.on(lis[i],'click',function(event){ 
                event = Vcity._m.getEvent(event);
                var target = Vcity._m.getTarget(event);
                document.getElementById('switchCity').innerText = Vcity.regExChiese.exec(target.innerHTML)[0];
